...
/*   Updated: 2024/01/11 22:46:00 by tunsal           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*gnl_bzero(void *s, size_t n)
{
	size_t	i;
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE < 1 || read(fd, stash[fd], 0) < 0)
		return (gnl_bzero(stash[fd], BUFFER_SIZE + 1), NULL);
	line = NULL;
	...
	while (read_count == BUFFER_SIZE && !ft_strchr(stash[fd], '\n'))
	{
		...
		if (read_count == -1)
			return (gnl_bzero(stash, BUFFER_SIZE + 1), NULL);
